 ...
 * @access public
 */
	function phone($check, $regex = null, $country = 'chn') {
		$_this =& Validation::getInstance();
		$_this->check = $check;
		$_this->regex = $regex;
		...
		if (is_null($_this->regex)) {
			switch ($_this->country) {				
				case 'chn':
					$_this->regex = '/^((\+[1-9]{1,4})|([1-9]{1,3}))?(1)\d{10}$/';//[86]13888888888 || [+86]13888888888
					break;
				case 'us':
				case 'all':
				case 'can':
...
/**
 * Check that a value is a valid telepnone number, such as [area code ]-telphone number-[telephone Extension]
 *
 * @param mixed $check Value to check (string or array)
 * @param string $regex Regular expression to use
 * @param string $country Country code (defaults to 'all')
 ...
 */
	function tel( $check, $regex = NULL ){		
		...
		if ( is_null( $_this->regex ) ) {
			$_this->regex = '/(^(0[0-9]{2,3}\-)?([2-9][0-9]{6,7}){1}(\-[0-9]{1,6})?$)|((^(400)|^(800))([0-9]{7,8})$)/';
		}
		if (empty($_this->regex)) {
			return $_this->_pass('tel', $check );
		}
		return $_this->_check();
	}

/**
 * Check that a value is a valid telepnone number
 *
 * @param mixed $check Value to check (string or array)
 * @param string $regex Regular expression to use
 * @param string $country Country code (defaults to 'all')
 ...
 */
	function qq( $check, $regex = NULL ){		
		...
		if ( is_null( $_this->regex ) ) {
			$_this->regex = '/^[1-9][0-9]{4,12}$/';
		}
		if (empty($_this->regex)) {
			return $_this->_pass('qq', $check );
		}
		return $_this->_check();
	}
 ...
 * @access public
 */
	function postal($check, $regex = null, $country ='chn') {
		$_this =& Validation::getInstance();
		$_this->check = $check;
		$_this->regex = $regex;
		$_this->country = $country;
