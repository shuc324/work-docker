<?php
/* SVN FILE: $Id: basics.php 8852 2011-09-15 09:52:20Z lightma $ */
/**
 ...
 * @version			$Revision: 8852 $
 * @modifiedby		$LastChangedBy: lightma $
 * @lastmodified	$Date: 2011-09-15 17:52:20 +0800 (四, 2011-09-15) $
...
//全局错误处理函数，在error_reporting设置为0时，也能捕获fatal error类错误，记录至apache
register_shutdown_function('global_error_logger');
function global_error_logger()
{
	$last_error = error_get_last();
	if (!is_null($last_error))
	{
		if (in_array($last_error['type'], array(E_USER_ERROR, E_COMPILE_ERROR, E_ERROR))) {
			error_log(json_encode($last_error), 0);
		}
	}
}
/**
...
if (!function_exists('loadModel')) {
	function loadModel($name = null, $create = false) {
		if(!class_exists('Model')){
			require LIBS . 'model' . DS . 'model.php';
		}
		if (!class_exists('AppModel')) {
		...
		if (!is_null($name) && !class_exists($name)) {
			...
			foreach($paths->modelPaths as $path) {
				if (file_exists($path . $name . '.php')) {
					...
					return $create ? new $className : true;
				}
			}
			return false;
		} else {
...
function loadControllers() {
	...
	if (!class_exists('AppController')) {
		if (file_exists(APP . 'app_controller.php')) {
			...
			require(COLA . 'app_controller.php');
		}
	}
	...
	foreach($paths->controllerPaths as $path) {
		foreach(listClasses($path) as $controller) {
			...
			$className = Inflector::camelize($name);
			$name = str_replace ('_controller.php', '', $controller);
...
function loadServices() {
	...
	if (!class_exists('AppService')) {
		if (file_exists(APP . 'app_service.php')) {
			require(APP . 'app_service.php');
		}
	}
	$loadedServices = array();

	foreach($paths->servicePaths as $path) {
		foreach(listClasses($path) as $service) {
			list($name) = explode('.', $service);
			$className = Inflector::camelize($name);
			$name = str_replace ('_service.php', '', $service);
			if (loadService($name)) {
				$loadedServices[$service] = $className;
			}
		}
	}
 ...
 */
	function loadService($name = null, $create = false) {
		...
		if (!is_null($name) && !class_exists($className)) {
			$name = Inflector::underscore($name);
			$file = $name . ($hasService ? '.php' : '_service.php');
			$paths = Configure::getInstance();
...
function clearCache($params = null, $type = 'views', $ext = '.php') {
	if (is_string($params) || $params === null) {
		...
		if (is_file($cache . $ext) || is_file($cache . '__fastapp' . $ext)) {
			@unlink($cache . $ext);
			//mic: clean fast app cache either
			@unlink($cache . '__fastapp' . $ext);
			return true;
...
function LogError($message) {
	...
	$bad = array("\n", "\r", "\t");
	$good = ' ';
	ColaLog::write('error', str_replace($bad, $good, $message));
}
/**
...
function ddebug() {
	global $TIME_START;
	echo "" . round(microtime(true) - $TIME_START, 6) * 1000 . "ms";
	die();
}
function cola_auto_session() {
	if (defined('AUTO_SESSION') && AUTO_SESSION) {
		if (!isset($_SESSION)) {
			if (!defined('COLA_SESSION_SAVE')) {
				define('COLA_SESSION_SAVE', 2 );	//默认为文件
			}			
			...
			if (function_exists('ini_set')) {
				...
				if (!defined('COLA_SESSION_COOKIE')) {
					define('COLA_SESSION_COOKIE', 'COLAPHP');
				}
				ini_set('session.name', COLA_SESSION_COOKIE);
				ini_set('session.cookie_path', '/');
				ini_set('session.auto_start', 0);
				
				//memcache
				if ( COLA_SESSION_SAVE == 1 ){
					if (!defined('COLA_SESSION_SAVE_PATH')) {
						define('COLA_SESSION_SAVE_PATH', MEM_CACHED_IP . ':' . MEM_CACHED_PORT);
					}
					ini_set('session.save_handler', "memcached");
					ini_set('session.save_path', COLA_SESSION_SAVE_PATH );					
				} else if ( COLA_SESSION_SAVE == 2 ){ //lite(file)
					if (!defined('COLA_SESSION_SAVE_PATH')) {
						define('COLA_SESSION_SAVE_PATH', TMP . 'sessions');
					}
					ini_set('session.save_path', COLA_SESSION_SAVE_PATH);
				}				
			}			
