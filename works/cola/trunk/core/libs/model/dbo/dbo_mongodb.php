...
class DboMongodb extends DboSource {
	...
	public function connect() {
		...
		try {
			...
			if (isset ( $this->config ['replicaset'] ) && count ( $this->config ['replicaset'] ) === 2) {
				...
				$this->connection = new $class ( $host, array (
	...
	public function insertMulti($table, $fields, $values) {
		...
		try {
			$return = $this->_db->selectCollection ( $table )->batchInsert ( $data, array (
				'w' => 1 
			) );
		} catch ( MongoException $e ) {
			$this->error = $e->getMessage ();
...
// 	public function create(Model $Model, $fields = null, $values = null) {
		...
		try {
			if ($this->_driverVersion >= '1.3.0') {
				$return = $this->_db->selectCollection ( $table )->insert ( $data, array (
					'w' => 1 //'safe' => true 
				) );
			} else {
				$return = $this->_db->selectCollection ( $table )->insert ( $data, true );
			}
		} catch ( MongoException $e ) {
			$this->error = $e->getMessage ();
			trigger_error ( $this->error );
	...
	public function ensureIndex(&$Model, $keys = array(), $params = array()) {
		...
		try {
			$return = $this->_db->selectCollection ( $table )->ensureIndex ( $keys, $params );
		} catch ( MongoException $e ) {
			$this->error = $e->getMessage ();
			trigger_error ( $this->error );
		}
...
// 	public function update(Model $Model, $fields = null, $values = null, $conditions = null) {
    public function update(&$Model, $fields = null, $values = null, $conditions = null) {
		...
		if (! empty ( $data ['_id'] )) {
			...
			try {
				if ($this->_driverVersion >= '1.3.0') {
					$return = $mongoCollectionObj->update ( $cond, $data, array (
						"multiple" => false, 
						'w' => 1 //'safe' => true 
					) );
				} else {
			...
			try {
				if ($this->_driverVersion >= '1.3.0') {
					$return = $mongoCollectionObj->save ( $data, array (
						'w' => 1 //'safe' => true 
					) );
				} else {
	...
	public function updateAll(&$Model, $fields = null, $conditions = null) {
		...
		try {
			if ($this->_driverVersion >= '1.3.0') {
				// not use 'upsert'
				$return = $this->_db->selectCollection ( $table )->update ( $conditions, $fields, array (
					"multiple" => true, 
					'w' => 1 //'safe' => true 
				) );
...
// 	public function read(Model $Model, $query = array(), $recursive = null) {
	public function read(&$Model, $query = array(), $recursive = null) {
		...
		if (empty ( $modify )) {
			...
			$return = $this->_db->selectCollection ( $table )->find ( $conditions, $fields )->sort ( $order )->limit ( $limit )->skip ( $offset );
			if (! empty ( $hint )) {
				$return->hint ( $hint );
			}
			if ($this->fullDebug) {
	...
	public function execute($query, $options = array(), $params = array()) {
		...
		$this->_prepareLogQuery ( $Model ); // just sets a timer
		$return = $this->_db->execute ( $query, $params );
		if ($this->fullDebug) {
			if ($params) {
