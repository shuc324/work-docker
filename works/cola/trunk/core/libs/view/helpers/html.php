 ...
 */
	function selectTag($fieldName, $optionElements, $selected = null, $selectAttr = array(), $optionAttr = null, $showEmpty = true, $return = false) {
		...
		if (isset($selectAttr) && array_key_exists("multiple", $selectAttr)) {
			$select[] = sprintf($this->tags['selectmultiplestart'], "[{$this->model}][".implode('][',explode('/',$this->field))."]", $this->parseHtmlOptions($selectAttr));
		} else {
			$select[] = sprintf($this->tags['selectstart'], "[{$this->model}][".implode('][',explode('/',$this->field))."]", $this->parseHtmlOptions($selectAttr));
		}
 ...
 */
	function tagErrorMsg($field, $text = '') {
		...
		if ( !empty($error) ) {
			...
			if ( $arg_num == 1 ) 
			{
				$text = $this->validationErrors[$this->model][$this->field];
			}
 ...
 */
	function setFormTag($tagValue) {
		return list($this->model, $this->field) = explode("/", $tagValue, 2);
	}
/**
