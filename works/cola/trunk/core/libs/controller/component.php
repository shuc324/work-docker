 ...
 */
	function init(&$controller) {
		...
		if (!empty($this->__controller->components)) {
			$loaded = array();
 ...
 */
	function &__loadComponents(&$loaded, $components) {
		foreach($components as $component) {
			//by lightma 2010-11-02，delete all plugin
			/*
			...
			if (in_array($component, array_keys($loaded)) !== true) {
				...
				if (!class_exists($componentCn)) {
					if (!loadComponent($component)) {
						$this->colaError('missingComponentFile', array(array(
					...
					if (!class_exists($componentCn)) {
						$this->colaError('missingComponentClass', array(array(
												'className' => $this->__controller->name,
												'component' => $component,
												'file' => Inflector::underscore($component) . '.php',
												'base' => $this->__controller->base)));
