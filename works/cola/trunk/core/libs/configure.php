<?php
/* SVN FILE: $Id: configure.php 7526 2011-07-06 02:03:11Z sparkwang $ */
/**
...
/**
 * Used to determine the current version of ColaPHP
 *
 * Usage Configure::version();
 *
 * @return string Current version of ColaPHP
 * @access public
 */
	function version() {
		$_this =& Configure::getInstance();
		if(!isset($_this->Cola['version'])){
			require(CORE_PATH . 'cola' . DS . 'config' . DS . 'config.php');
			$_this->write($config);
		}
		return $_this->Cola['version'];
	}
/**
...
/**
 * Sets the var servicePaths
 *
 * @param array $servicePaths
 * @access private
 */
	function __buildServicePaths($servicePaths) {
		$_this =& Configure::getInstance();
		$_this->servicePaths[] = SERVICES;
		if (isset($servicePaths)) {
			foreach($servicePaths as $value) {
				$_this->servicePaths[] = $value;
			}
		}
	}
/**
 ...
 */
	function __loadBootstrap() {
		...
		$_this->__buildServicePaths($servicePaths);
