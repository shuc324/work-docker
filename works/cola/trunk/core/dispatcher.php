<?php
/* SVN FILE: $Id: dispatcher.php 7781 2011-07-26 02:54:19Z lightma $ */
/**
 * Dispatcher takes the URL information, parses it for paramters and
 * tells the involved controllers what to do.
 *
 * This is the heart of Cola's operation.
 *
 ...
 * @version			$Revision: 7781 $
 * @modifiedby		$LastChangedBy: lightma $
 * @lastmodified	$Date: 2011-07-26 10:54:19 +0800 (二, 2011-07-26) $
 * @license			http://www.opensource.org/licenses/mit-license.php The MIT License
 */
/**
 * List of helpers to include
 */
uses('router', DS.'controller'.DS.'controller', DS.'service'.DS.'service_base');
/**
...
class Dispatcher extends Object {
	...
	function dispatch($url, $additionalParams=array()) {
		...
		if (empty($params['controller'])) {
			...
			if (!loadController($ctrlName)) {
				if(preg_match('/([\\.]+)/', $ctrlName)) {
					return $this->colaError('error404', array(array('url' => strtolower($ctrlName), 'message' => 'Was not found on this server', 'base' => $this->base)));
		...
		if(defined('COLA_ADMIN')) {
			if(isset($params[COLA_ADMIN])) {
				$this->admin = '/'.COLA_ADMIN ;
				$url = preg_replace('/'.COLA_ADMIN.'\//', '', $url);
				$params['action'] = COLA_ADMIN.'_'.$params['action'];
			} elseif (strpos($params['action'], COLA_ADMIN) === 0) {
				$privateAction = true;
			}
		}

		if ($missingController) {
			return $this->colaError('missingController', array(
		...
		if (in_array(strtolower($params['action']), array('tostring', 'requestaction', 'log', 'colaerror', 'constructclasses', 'redirect', 'set', 'setaction', 'validate', 'validateerrors', 'render', 'referer', 'flash', 'flashout', 'generatefieldnames', 'postconditions', 'cleanupfields', 'beforefilter', 'beforerender', 'afterfilter'))) {
			$missingAction = true;
		}
		...
		if ($missingAction && !in_array('scaffold', array_keys($classVars))) {
			...
			return $this->colaError('missingAction', array(
			array('className' => Inflector::camelize($params['controller']."Controller"), 'action' => $params['action'], 'webroot' => $this->webroot, 'url' => $url, 'base' => $this->base)));
		}

		if ($privateAction) {
			$this->start($controller);
			return $this->colaError('privateAction', array(
			array('className' => Inflector::camelize($params['controller']."Controller"), 'action' => $params['action'], 'webroot' => $this->webroot, 'url' => $url, 'base' => $this->base)));
		}
	...
	function parseParams($from_url) {
		$Route = new Router();
		$routes_mtime = filemtime(CONFIGS.'routes.php');
		//check whether the cache exists
		$cache_name = 'persistents'. DS . 'cache_routes_'.$routes_mtime;
		if (file_exists(CACHE . $cache_name))
		{
			$data = cache($cache_name);
			if (!empty($data))
			{				
				$Route->routes = unserialize($data);
			}
		}
		if (empty($Route->routes))
		{
			//delete all cache_route_*
			$cache_files = glob(CACHE . 'persistents' . DS . 'cache_routes_*');
			foreach ($cache_files as $file)
			{				
				unlink($file);
			}
			include CONFIGS.'routes.php';
			$Route->connect('/bare/:controller/:action/*', array('bare' => '1'));
			$Route->connect('/ajax/:controller/:action/*', array('bare' => '1'));
			$data = serialize($Route->routes);
			cache($cache_name, $data, '2048-01-01');
		}
