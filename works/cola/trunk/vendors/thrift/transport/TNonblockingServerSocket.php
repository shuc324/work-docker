...
class TNonblockingServerSocket extends TServerTransport {
	 ...
	 * Start listening
	 */
	public function listen($block = true) {
		$this->handle_ = stream_socket_server("tcp://{$this->host_}:{$this->port_}", $this->errno_, $this->errstr_);
		if (!$this->handle_) {
			throw new Exception("Can not create the server socket!");
		}
	...
	public function loop($block_once = false) {
		if ($block_once) {
			event_base_loop($this->base_, EVLOOP_ONCE);
		} else {
			event_base_loop($this->base_, EVLOOP_NONBLOCK);
		}
	}
