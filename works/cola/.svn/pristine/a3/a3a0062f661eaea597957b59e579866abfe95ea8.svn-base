<?php
/* SVN FILE: $Id: dbo_source.php 6430 2011-04-28 03:24:04Z jeriwu $ */
/**
 ...
 * @since			ColaPHP(tm) v 0.10.0.1076
 * @version			$Revision: 6430 $
 * @modifiedby		$LastChangedBy: jeriwu $
 * @lastmodified	$Date: 2011-04-28 11:24:04 +0800 (四, 2011-04-28) $
...
class DboSource extends DataSource {
	...
	function __construct($config = null, $autoConnect = true) {
		...
		if ($autoConnect && empty($config['mirrors'])) {
			return $this->connect();
		} else {
			return true;
		}
	...
	function value($data, $column = null) {
		...
		if (is_array($data)) {
			...
			$count = count($data);
			for($i = 0; $i < $count; $i++) {
				$out[$keys[$i]] = $this->value($data[$keys[$i]], $column);
			}
			return $out;
		} elseif (in_array($data, array('{$__colaID__$}', '{$__colaForeignKey__$}'), true)) {
			return $data;
		} else {
			return null;
		}
	...
	/**
	 * connected to database for read/write seperate, db balance, backup for db disater recovery
	 *
	 * @param string $sql
	 * @return boolean
	 */
	function _selectDbMirror($sql) {
		if (empty($this->config['mirrors'])) {
			return true;
		}

		if (!preg_match('/^(select|describe|set)\s+/i', $sql)) {
			if (isset($this->config['__HOST']) && $this->config['__HOST'] != $this->config['host']) {
				$this->config['host'] = $this->config['__HOST'];
		...
		if (is_string($this->config['mirrors'])) {
			$this->config['mirrors'] = array($this->config['mirrors']);
		}
		if (!isset($this->config['__HOST'])) {
			$this->config['__HOST'] = $this->config['host'];
		}
		$this->config['host'] = $this->__selectDbMirrorBalance($this->config['__HOST'], $this->config['mirrors']);

		ob_start();
		$connected = $this->reconnect();
		ob_end_clean();

		$cachedDbMirrors = $this->__listDbMirrors();
		$hostStatus = @$cachedDbMirrors[$this->config['host']];
		$hostStatus = empty($hostStatus) ? array() : $hostStatus;
		$hostStatus['last'] = time();
		$hostStatus['failed'] = (isset($hostStatus['failed']) ? $hostStatus['failed'] : 0) + ($connected ? 0 : 1);
		$cachedDbMirrors[$this->config['host']] = $hostStatus;
		
		$this->__listDbMirrors($cachedDbMirrors);
			
		if (!$connected) {
			return $this->_selectDbMirror($sql);
		}
	...
	/**
	 * list cached db mirrors
	 *
	 * @param mixed $data
	 * @return array
	 */
	function __listDbMirrors($data = null) {
		if (empty($data) && $this->__dbMirrors != null) {
			return $this->__dbMirrors;
		}
		...
		$filename = ConnectionManager::getSourceName($this) . '_' . $this->config['database'] . '_mirrors';
		$new = cache('models' . DS . $filename, $data, $expires);

		if ($new != null) {
			$new = unserialize($new);
			$this->__dbMirrors = $new;
		}
		return $new ? $new : array();
	}

	/**
	 * random select db mirror
	 *
	 * @param string $host
	 * @param array $mirrors
	 * @return string
	 */
	function __selectDbMirrorBalance($host, $mirrors) {
		if (!in_array($host, $mirrors)) {
			$mirrors[] = $host;
		}
		$index = rand(0, count($mirrors) - 1);
		$selectedHost = $mirrors[$index];
		$cachedDbMirrors = $this->__listDbMirrors();
		if (isset($cachedDbMirrors[$selectedHost])) {
			$hostStatus = $cachedDbMirrors[$selectedHost];
			if ($hostStatus['failed'] > 0) {
				if ((time() - $hostStatus['last']) < $hostStatus['failed'] * 60) {
					$index = $index + 1;
					$index = $index >= count($mirrors) ? 0 : $index;
	...
	function showLog($sorted = false) {
		...
		if (php_sapi_name() != 'cli') {
			print ("<table class=\"colaSqlLog\" id=\"colaSqlLog_" . preg_replace('/[^A-Za-z0-9_]/', '_', uniqid(time(), true)) . "\" summary=\"Cola SQL Log\" cellspacing=\"0\" border = \"0\">\n<caption>{$this->_queriesCnt} {$text} took {$this->_queriesTime} ms</caption>\n");
	 ...
	 * @return unknown
	 */
	function generateQuery(&$model, &$queryData) {
		$this->__scrubQueryData($queryData);
		$joinedOnSelf = false;
	 ...
	 * @return array
	 */
	function _fields(&$model, $alias = null, $fields = array(), $quote = true) {
		if (empty($alias)) {
			$alias = $model->name;
		}
		...
		if ($count >= 1 && !in_array($fields[0], array('*', 'COUNT(*)'))) {
			for($i = 0; $i < $count; $i++) {
				if (!preg_match('/^.+\\(.*\\)/', $fields[$i])) {
					...
					$tmp_field = strtoupper( $fields[$i] );
					if (strpos($tmp_field, 'DISTINCT ') === 0 || strpos($tmp_field, 'DISTINCT	') === 0) {
						$prepend   = 'DISTINCT ';
						$fields[$i] = trim( substr( $fields[$i], strlen( 'DISTINCT' ), strlen($fields[$i]) - strlen( 'DISTINCT' ) ) );						
					}
					$dot = strpos($fields[$i], '.');
					if ($dot === false) {
	...
	function conditionKeysToString($conditions, $quoteValues = true, &$model=null) {
		...
		foreach($conditions as $key => $value) {
			...
			}else if(in_array(strtolower(trim($key)), $operate)){//增加大小比较 $query['>'] = " created_microtime > 1234 ";
				$out[] =  $value;
			}else {
				if (is_string($value) && preg_match('/^\{\$__colaIdentifier\[(.*)\]__\$}$/', $value, $identifier) && isset($identifier[1])) {
						...
						if	(strpos($value[0], '-!') === 0){
							...
							if ($quoteValues) {
								...
								foreach($value as $valElement) {
									$data .= $this->value($valElement, $type) . ', ';
								}
							}
							$data[strlen($data) - 2] = ')';
						}
					...
					if (strpos($match['2'], '-!') === 0) {
						...
						if (!empty($match['2']) && $quoteValues) {
							if(!preg_match('/[A-Za-z]+\\([a-z0-9]*\\),?\\x20+/', $match['2'])) {
								...
								$match['2'] = $this->value($match['2'], $type);
							}
