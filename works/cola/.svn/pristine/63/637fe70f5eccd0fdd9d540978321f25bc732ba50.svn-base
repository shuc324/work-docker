<?php
/* SVN FILE: $Id: dbo_mysql.php 4779 2010-11-26 09:27:47Z lightma $ */
/**
 * MySQL layer for DBO
 *
 ...
 *
 * @package			cola
 * @subpackage		cola.core.libs.model.dbo
 * @since			ColaPHP(tm) v 0.10.5.1790
 * @version			$Revision: 4779 $
 * @modifiedby		$LastChangedBy: lightma $
 * @lastmodified	$Date: 2010-11-26 17:27:47 +0800 (五, 2010-11-26) $
 * @license			http://www.opensource.org/licenses/mit-license.php The MIT License
 */

if (!defined('MYSQL_CONNECT_TIMEOUT')) {
	define('MYSQL_CONNECT_TIMEOUT', 5);
}
 ...
 */
	function connect() {
		...
		if (!$config['persistent'] || $config['connect'] === 'mysql_connect') {
			...
			if ($connect == 'mysqli_connect') {
				//host,username,password,dbname,port,socket
				$this->connection = $connect($config['host'], $config['login'], $config['password'], $config['database'], $config['port']);
		...
		if ($this->connected) {
			...
			if (isset($config['serverid']) && !empty($config['serverid'])) {
				$this->setReplication($config['serverid']);
			}
		}
	...
	function setReplication($server_id) {
		$this->execute('set auto_increment_increment = 10');
		$this->execute('set auto_increment_offset = '.$server_id);
 	}
/**
 ...
 */
	function listSources($data = null) {
		...
		$result = $this->execute('SHOW TABLES FROM ' . $this->name($this->config['database']) . ';');
 ...
 */
	function value($data, $column = null, $safe = false) {
		...
		switch ($column) {
			...
			default:
				//$data = "'" . ($this->connection ? mysql_real_escape_string($data, $this->connection) : mysql_escape_string($data)) . "'";
			    $data = "'" . ($this->connection ? mysqli_real_escape_string($this->connection, $data) : mysql_escape_string($data)) . "'";
			break;
		}
	...
	function lastError() {
		...
		*/
		if (mysqli_errno($this->connection)) {
			return mysqli_errno($this->connection).': '.mysqli_errno($this->connection);
		}
		return null;
	}
/**
 ...
 */
	function resultSet(&$results) {
		...
		while ($j < $num_fields) {
			...
			$column = mysqli_fetch_field($results);
			if(is_object($column)) {
 ...
 *
 * @param object $schema An instance of a subclass of ColaSchema
 * @param string $table Optional.  If specified only the table name given will be generated.
 *                      Otherwise, all tables defined in the schema are generated.
 * @return string
 */
	function generateSchema($schema, $table = null) {
		if (!is_a($schema, 'ColaSchema')) {
