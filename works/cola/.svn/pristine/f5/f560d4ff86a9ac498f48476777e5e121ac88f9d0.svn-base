...
class DboMongodb extends DboSource {
	...
	public $columns = array (
		...
		'datetime' => array (
			'name' => 'datetime', 
			'format' => null, 
			'formatter' => 'MongodbDateFormatter' 
		), 
		'timestamp' => array (
			'name' => 'timestamp', 
			'format' => null, 
	...
	public function connect() {
		...
		try {
			...
			if (isset ( $this->config ['replicaset'] ) && count ( $this->config ['replicaset'] ) === 2) {
                ...
                //$MongoClient = new MongoDB\Client($uri = 'mongodb://localhost:27017', array $uriOptions = [], array $driverOptions = [])
				$this->connection = new MongoDB\Client ( $host, $uriOptions=array(), $driverOptions );
			}
	...
	public function insertMulti($table, $fields, $values) {
		...
		try {
			$return = $this->_db->selectCollection ($table )->insertMany ( $data, array (
				'w' => 1
			) );
		} catch ( MongoException $e ) {
			$this->error = $e->getMessage ();
	...
	public function describe($Model) {
		...
		if (! empty ( $Model->mongoSchema ) && is_array ( $Model->mongoSchema )) {
			...
			/*
			return $schema + array (
				$Model->primaryKey => $this->_defaultSchema ['_id'] 
			);
			*/
			$arr = $schema + $this->_defaultSchema;
			$schema = array ();
			foreach ( $arr as $col => $desc ) {
				$desc ['name'] = $col;
				$schema [] = $desc;
			}
			return $schema;
...
// 	public function create(Model $Model, $fields = null, $values = null) {
		...
		try {
			if ($this->_driverVersion >= '1.3.0') {
				$return = $this->_db->selectCollection ( $table )->insertOne ( $data, array (
					'w' => 1 //'safe' => true
				) );
			} else {
				$return = $this->_db->selectCollection ( $table )->insertOne ( $data, true );
			}
		} catch ( MongoException $e ) {
			$this->error = $e->getMessage ();
			trigger_error ( $this->error );
		...
		if (! empty ( $return ) && $return->getInsertedId()) {
			$id = $return->getInsertedId()->__toString();
	...
	public function distinct(&$Model, $keys = array(), $params = array()) {
		...
		try {
			$return = $this->_db->selectCollection ( $table )->distinct ( $keys, $params );
		} catch ( MongoException $e ) {
			$this->error = $e->getMessage ();
			trigger_error ( $this->error );
		}
	 ...
	 * @access public
	 */
    //TODO collection not support group
	public function group($params, $Model = null) {
		...
		try {
			$return = $this->_db->selectCollection ( $table )->group ( $key, $initial, $reduce, $options );
		} catch ( MongoException $e ) {
			$this->error = $e->getMessage ();
			trigger_error ( $this->error );
		}
	...
	public function ensureIndex(&$Model, $keys = array(), $params = array()) {
		...
		try {
			$return = $this->_db->selectCollection ( $table )->createIndex ( $keys, $params );
		} catch ( MongoException $e ) {
			$this->error = $e->getMessage ();
			trigger_error ( $this->error );
		}
...
// 	public function update(Model $Model, $fields = null, $values = null, $conditions = null) {
    public function update(&$Model, $fields = null, $values = null, $conditions = null) {
		...
		if (! empty ( $data ['_id'] )) {
			...
			try {
				if ($this->_driverVersion >= '1.3.0') {
					$return = $mongoCollectionObj->updateOne ( $cond, $data, array (
						"multiple" => false, 
						'w' => 1 //'safe' => true
					) );
				} else {
			...
			if ($this->fullDebug) {
				$this->logQuery ( "db.{$table}.update( :conditions, :data, :params )", array (
					'conditions' => $cond, 
					'data' => $data, 
					'params' => array (
						"multiple" => false 
			...
			try {
				if ($this->_driverVersion >= '1.3.0') {
					$return = $mongoCollectionObj->insertOne ( $data, array (
						'w' => 1 //'safe' => true
					) );
				} else {
					$return = $mongoCollectionObj->insertOne ( $data );
				}
			} catch ( MongoException $e ) {
				$this->error = $e->getMessage ();
				trigger_error ( $this->error );
	...
	public function updateAll(&$Model, $fields = null, $conditions = null) {
		...
		try {
			if ($this->_driverVersion >= '1.3.0') {
				// not use 'upsert'
				$return = $this->_db->selectCollection ( $table )->updateMany ( $conditions, $fields, array (
					"multiple" => true,
					'w' => 1 //'safe' => true
				) );
		...
		if ($this->fullDebug) {
			$this->logQuery ( "db.{$table}.update( :conditions, :fields, :params )", array (
...
// 	public function delete(Model $Model, $conditions = null) {
    public function delete(&$Model, $conditions = null) {
		...
		try {
			$this->_prepareLogQuery ( $Model ); // just sets a timer
			$return = $mongoCollectionObj->deleteOne ( $conditions );
			if ($this->fullDebug) {
				$this->logQuery ( "db.{$table}.remove( :conditions )", compact ( 'conditions' ) );
			}
			$return = true;
		} catch ( MongoException $e ) {
			$this->error = $e->getMessage ();
	 ...
	 * @access public
	 */
	public function findCount(Model $Model, $conditions = array()) {
		if (! $this->isConnected ()) {
			return false;
		}
		$this->_stripAlias ( $conditions, $Model->alias );
		...
		$table = $this->fullTableName ( $Model );
		$return = $this->_db->selectCollection ( $table )->count ( $conditions );
		return $return;
	}
...
// 	public function read(Model $Model, $query = array(), $recursive = null) {
	public function read(&$Model, $query = array(), $recursive = null) {
		...
		$table = $this->fullTableName ( $Model );
		if (empty ( $modify )) {
			if ($Model->findQueryType === 'count' && $fields == array (
				'count' => true
			)) {
				$cursor = $this->_db->selectCollection ( $table )->find ( $conditions, array (
					'_id' => true
				) );
				if (! empty ( $hint )) {
					$cursor->hint ( $hint );
				...
				if ($this->fullDebug) {
					...
					$this->logQuery ( "db.{$table}.find( :conditions ).hint( :hint ).count()", compact ( 'conditions', 'count', 'hint' ) );
				}
				return array (
					array (
						$Model->alias => array (
			...
			$return = $this->_db->selectCollection ( $table )->find ( $conditions, $options );
			if (! empty ( $hint )) {
				$return->hint ( $hint );
			}
			if ($this->fullDebug) {
//				$count = $return->count ( true );
				...
				$this->logQuery ( "db.{$table}.find( :conditions, :fields ).sort( :order ).limit( :limit ).skip( :offset ).hint( :hint )", compact ( 'conditions', 'fields', 'order', 'limit', 'offset', 'count', 'hint' ) );
			}
		} else {
			...
			if ($this->fullDebug) {
				...
				$this->logQuery ( "db.runCommand( :options )", array (
		...
		if (is_object ( $return )) {
			$_return = array ();
            $it = new \IteratorIterator($return);
            $it->rewind(); // Very important
            while($mongodata = $it->current()) {
	...
	public function truncate($table) {
		...
		$return = false;
		try {
			$return = $this->getMongoDb ()->selectCollection ( $fullTableName )->deleteMany ( array () );
			if ($this->fullDebug) {
	...
	public function runCommand($command){
		...
		if ($this->fullDebug) {
			$this->logQuery ( "db.runCommand( :command )", compact ( 'command' ) );
		}
	...
	public function query() {
		...
		if (count ( $args ) > 1 && (strpos ( $args [0], 'findBy' ) === 0 || strpos ( $args [0], 'findAllBy' ) === 0)) {
			...
			if (substr ( $args [0], 0, 6 ) === 'findBy') {
				...
				return $args [2]->find ( 'first', array (
					'conditions' => array (
						$field => $args [1] [0] 
					) 
				) );
	...
	public function execute($query, $options = array(), $params = array()) {
		...
		$this->_prepareLogQuery ( $Model ); // just sets a timer
		$return = $this->_db->command ( $query, $params );
		if ($this->fullDebug) {
			if ($params) {
...
//			$mixed = new MongoId ( $mixed );
	...
	protected function _stringify(&$args = array(), $level = 0) {
		foreach ( $args as &$arg ) {
			...
			} elseif (is_object ( $arg ) && is_callable ( array (
				$arg, 
				'__toString' 
			) )) {
				$class = get_class ( $arg );
