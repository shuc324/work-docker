<?php
/* SVN FILE: $Id: view.php 3846 2010-09-27 03:48:20Z lightma $ */
/**
 ...
 *
 * @package			cola
 * @subpackage		cola.core.libs.view
 * @since			ColaPHP(tm) v 0.10.0.1076
 * @version			$Revision: 3846 $
 * @modifiedby		$LastChangedBy: lightma $
 * @lastmodified	$Date: 2010-09-27 11:48:20 +0800 (一, 2010-09-27) $
 ...
 */
	var $layout = 'default';
/**
 * Turns on or off Cola's conventional mode of rendering views. On by default.
 *
 * @var boolean
 * @access public
 */
	var $autoRender = true;
/**
 * Turns on or off Cola's conventional mode of finding layout files. On by default.
 *
 * @var boolean
 * @access public
 */
 ...
 */
	var $loaded = array();
/**
 * File extension. Defaults to Cola's conventional ".thtml".
 *
 * @var array
 * @access public
 */
 ...
 * @access protected
 */
	var $_passedVars = array('viewVars', 'action', 'autoLayout', 'autoRender', 'ext', 'base', 'webroot', 'helpers', 'here', 'layout', 'modelNames', 'name', 'pageTitle', 'viewPath', 'params', 'data', 'webservices');
/**
 * Constructor
 *
	...
	function render($action = null, $layout = null, $file = null) {
		...
		if (!is_file($viewFileName) && !fileExistsInPath($viewFileName) || $viewFileName === '/' || $viewFileName === '\\') {
			...
			if (strpos($action, 'missingView') === false) {
				return $this->colaError('missingView', array(array('className' => $this->controller->name,
																					'action' => $action,
																					'file' => $viewFileName,
																					'base' => $this->base)));
			...
			if ($out !== false) {
				if ($this->layout && $this->autoLayout) {
					...
					if (isset($this->loaded['cache']) && ((isset($this->controller) && $this->controller->cacheAction != false)) && (defined('CACHE_CHECK') && CACHE_CHECK === true)) {
						$replace = array('<cola:nocache>', '</cola:nocache>');
 ...
 */
	function renderLayout($contentForLayout) {
		...
		if (is_file($layoutFilename)) {
			...
			return $this->colaError('missingLayout', array(array('layout' => $this->layout,
																					'file' => $layoutFilename,
																					'base' => $this->base)));
		}
	}
	...
	function _render($___viewFn, $___dataForView, $loadHelpers = true, $cached = false) {
		...
		if (isset($this->loaded['cache']) && ((isset($this->controller) && $this->controller->cacheAction != false)) && (defined('CACHE_CHECK') && CACHE_CHECK === true)) {
			if (is_a($this->loaded['cache'], 'CacheHelper')) {
				...
				$cache->here			= setUri(); //modified by spark, 090408, old:$this->here;
			...
			if (in_array($helper, array_keys($loaded)) !== true) {
				if (!class_exists($helperCn)) {
					if (!loadHelper($helper)) {
						$this->colaError('missingHelperFile', array(array(
												'helper' => $helper,
					...
					if (!class_exists($helperCn)) {
						$this->colaError('missingHelperClass', array(array(
												'helper' => $helper,
												'file' => Inflector::underscore($helper) . '.php',
												'base' => $this->base)));
						exit();
				...
				$vars = array('base', 'webroot', 'here', 'params', 'action', 'data', 'themeWeb');
				$c = count($vars);
				for ($j = 0; $j < $c; $j++) {
